...
def main() -> int:
    port = int(os.getenv('OPENHANDS_APP_SMOKE_PORT', '8022'))
    temp_dir = Path(tempfile.mkdtemp(prefix='devagent-openhands-smoke-'))
    config_path = temp_dir / 'agent-studio.json'
    ...
    os.environ.setdefault('OH_LIFESPAN_RUN_ALEMBIC_ON_STARTUP', 'false')
...
def wait_for_config(
    ...
    for _ in range(100):
        try:
            ...
            last_error = exc
            time.sleep(0.05)
    raise RuntimeError(f'OpenHands app smoke server did not start: {last_error}')
